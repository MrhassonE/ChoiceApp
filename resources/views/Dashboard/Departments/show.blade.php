<x-appDash-layout>
    <div class="container-fluid">
        <div class="row align-items-center">
            <div class="col-md-6">
                <div class="mb-3">
                    <h5 class="card-title">النشاطات<span class="text-muted fw-normal ms-2"> ({{$department->Activity->count()}})</span></h5>
                </div>
            </div>
            ...
            <div class="col-md-12">
                <div class="d-flex flex-wrap align-items-center justify-content-start gap-2 mb-3">
                    <div>
                        @if(\Illuminate\Support\Facades\Auth::user()->hasPermission('activity-create'))
        ...
        <!-- end row -->
        @if(\Illuminate\Support\Facades\Auth::user()->hasPermission('activity-read'))
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex justify-content-center">
                                <div>
                                    <img src="{{$activity->img_path}}" alt="" style="object-fit: cover;object-position: top" class="avatar-xl img-thumbnail">
                                </div>
                            </div>
                            <div class="mt-3 pt-1">
                                <p class="mb-0 mt-2">
                            ...
                            <div class="d-flex gap-2 pt-4">
                                @if(\Illuminate\Support\Facades\Auth::user()->hasPermission('activity-delete'))
                                <button type="button" onclick="DeleteContact({{$activity->id}})" class="btn btn-soft-danger btn-sm w-50"><i class="bx bx-trash me-1"></i> حذف</button>
                                @endif
                                @if(\Illuminate\Support\Facades\Auth::user()->hasPermission('activity-update'))
                                <a href="{{route('Activity.edit',$activity->id)}}"  class="btn btn-primary btn-sm w-50"><i class="bx bx-pencil me-1"></i> تعديل</a>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-dialog modal-xl modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="myExtraLargeModalLabel">أضافة نشاط</h5>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 my-1">
                                    <div class="form-group">
                                        ...
                                        <input id="url" name="url" maxlength="190" type="text" class="form-control" placeholder="ادخل الرابط">
                                        <div class="invalid-feedback">
                                            الرجاء املئ الحقل
                                        </div>
                                    </div>
    ...
    <script >
        ...
        $("#CreateForm").on('submit',function (event) {
            ...
            if (validateForm === true)
            {
                ...
                $.ajax({
                    type: 'post',
                    url : '{{route('Activity.store',$department->id)}}',
                    data: formData,
                    contentType:false,
                    processData:false,
                    success : function () {
